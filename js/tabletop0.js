(function() {
  ...
  /*
    Initialize with Tabletop0.init( { key: '0AjAPaAU9MeLFdHUxTlJiVVRYNGRJQnRmSnQwTlpoUXc' } )
      OR!
    Initialize with Tabletop0.init( { key: 'https://docs.google.com/spreadsheet/pub?hl=en_US&hl=en_US&key=0AjAPaAU9MeLFdHUxTlJiVVRYNGRJQnRmSnQwTlpoUXc&output=html&widget=true' } )
      OR!
    Initialize with Tabletop0.init('0AjAPaAU9MeLFdHUxTlJiVVRYNGRJQnRmSnQwTlpoUXc')
  */

  var Tabletop0 = function(options) {
    // Make sure Tabletop0 is being used as a constructor no matter what.
    if(!this || !(this instanceof Tabletop0)) {
      return new Tabletop0(options);
    }
    ...
    if (this.singleton) {
      if (typeof(Tabletop0.singleton) !== 'undefined') {
        this.log('WARNING! Tabletop0 singleton already defined');
      }
      Tabletop0.singleton = this;
    }
    ...
    if (!this.key) {
      this.log('You need to pass Tabletop0 a key!');
      return;
    }
  ...
  Tabletop0.init0 = function(options) {
    return new Tabletop0(options);
  };

  Tabletop0.sheets = function() {
    this.log('Times have changed! You\'ll want to use var Tabletop0 = Tabletop0.init(...); Tabletop0.sheets(...); instead of Tabletop0.sheets(...)');
  };
    ...
    injectScript: function(path, callback) {
      ...
      if (this.singleton) {
        if (callback === this.loadSheets) {
          callbackName = 'Tabletop0.singleton.loadSheets';
        } else if (callback === this.loadSheet) {
          callbackName = 'Tabletop0.singleton.loadSheet';
        }
      } else {
        var self = this;
        callbackName = 'tt' + (+new Date()) + (Math.floor(Math.random()*100000));
        Tabletop0.callbacks[ callbackName ] = function () {
          var args = Array.prototype.slice.call( arguments, 0 );
          callback.apply(self, args);
          script.parentNode.removeChild(script);
          delete Tabletop0.callbacks[callbackName];
        };
        callbackName = 'Tabletop0.callbacks.' + callbackName;
      }
  ...
  Tabletop0.Model = function(options) {
    var i, j, ilen, jlen;
    this.columnNames = [];
  ...
  if(typeof module !== 'undefined' && module.exports) {
    module.exports = Tabletop0;
  } else if (typeof define === 'function' && define.amd) {
    define(function () {
      return Tabletop0;
    });
  } else {
    window.Tabletop0 = Tabletop0;
  }
