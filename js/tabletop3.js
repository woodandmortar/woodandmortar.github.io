(function() {
  ...
  /*
    Initialize with Tabletop3.init( { key: '0AjAPaAU9MeLFdHUxTlJiVVRYNGRJQnRmSnQwTlpoUXc' } )
      OR!
    Initialize with Tabletop3.init( { key: 'https://docs.google.com/spreadsheet/pub?hl=en_US&hl=en_US&key=0AjAPaAU9MeLFdHUxTlJiVVRYNGRJQnRmSnQwTlpoUXc&output=html&widget=true' } )
      OR!
    Initialize with Tabletop3.init('0AjAPaAU9MeLFdHUxTlJiVVRYNGRJQnRmSnQwTlpoUXc')
  */

  var Tabletop3 = function(options) {
    // Make sure Tabletop3 is being used as a constructor no matter what.
    if(!this || !(this instanceof Tabletop3)) {
      return new Tabletop3(options);
    }
    ...
    if (this.singleton) {
      if (typeof(Tabletop3.singleton) !== 'undefined') {
        this.log('WARNING! Tabletop3 singleton already defined');
      }
      Tabletop3.singleton = this;
    }
    ...
    if (!this.key) {
      this.log('You need to pass Tabletop3 a key!');
      return;
    }
  ...
  Tabletop3.init3 = function(options) {
    return new Tabletop3(options);
  };

  Tabletop3.sheets = function() {
    this.log('Times have changed! You\'ll want to use var Tabletop3 = Tabletop3.init(...); Tabletop3.sheets(...); instead of Tabletop3.sheets(...)');
  };
    ...
    injectScript: function(path, callback) {
      ...
      if (this.singleton) {
        if (callback === this.loadSheets) {
          callbackName = 'Tabletop3.singleton.loadSheets';
        } else if (callback === this.loadSheet) {
          callbackName = 'Tabletop3.singleton.loadSheet';
        }
      } else {
        var self = this;
        callbackName = 'tt' + (+new Date()) + (Math.floor(Math.random()*100000));
        Tabletop3.callbacks[ callbackName ] = function () {
          var args = Array.prototype.slice.call( arguments, 0 );
          callback.apply(self, args);
          script.parentNode.removeChild(script);
          delete Tabletop3.callbacks[callbackName];
        };
        callbackName = 'Tabletop3.callbacks.' + callbackName;
      }
  ...
  Tabletop3.Model = function(options) {
    var i, j, ilen, jlen;
    this.columnNames = [];
  ...
  if(typeof module !== 'undefined' && module.exports) {
    module.exports = Tabletop3;
  } else if (typeof define === 'function' && define.amd) {
    define(function () {
      return Tabletop3;
    });
  } else {
    window.Tabletop3 = Tabletop3;
  }
