(function() {
  ...
  /*
    Initialize with Tabletop4.init( { key: '0AjAPaAU9MeLFdHUxTlJiVVRYNGRJQnRmSnQwTlpoUXc' } )
      OR!
    Initialize with Tabletop4.init( { key: 'https://docs.google.com/spreadsheet/pub?hl=en_US&hl=en_US&key=0AjAPaAU9MeLFdHUxTlJiVVRYNGRJQnRmSnQwTlpoUXc&output=html&widget=true' } )
      OR!
    Initialize with Tabletop4.init('0AjAPaAU9MeLFdHUxTlJiVVRYNGRJQnRmSnQwTlpoUXc')
  */

  var Tabletop4 = function(options) {
    // Make sure Tabletop4 is being used as a constructor no matter what.
    if(!this || !(this instanceof Tabletop4)) {
      return new Tabletop4(options);
    }
    ...
    if (this.singleton) {
      if (typeof(Tabletop4.singleton) !== 'undefined') {
        this.log('WARNING! Tabletop4 singleton already defined');
      }
      Tabletop4.singleton = this;
    }
    ...
    if (!this.key) {
      this.log('You need to pass Tabletop4 a key!');
      return;
    }
  ...
  Tabletop4.init4 = function(options) {
    return new Tabletop4(options);
  };

  Tabletop4.sheets = function() {
    this.log('Times have changed! You\'ll want to use var Tabletop4 = Tabletop4.init(...); Tabletop4.sheets(...); instead of Tabletop4.sheets(...)');
  };
    ...
    injectScript: function(path, callback) {
      ...
      if (this.singleton) {
        if (callback === this.loadSheets) {
          callbackName = 'Tabletop4.singleton.loadSheets';
        } else if (callback === this.loadSheet) {
          callbackName = 'Tabletop4.singleton.loadSheet';
        }
      } else {
        var self = this;
        callbackName = 'tt' + (+new Date()) + (Math.floor(Math.random()*100000));
        Tabletop4.callbacks[ callbackName ] = function () {
          var args = Array.prototype.slice.call( arguments, 0 );
          callback.apply(self, args);
          script.parentNode.removeChild(script);
          delete Tabletop4.callbacks[callbackName];
        };
        callbackName = 'Tabletop4.callbacks.' + callbackName;
      }
  ...
  Tabletop4.Model = function(options) {
    var i, j, ilen, jlen;
    this.columnNames = [];
  ...
  if(typeof module !== 'undefined' && module.exports) {
    module.exports = Tabletop4;
  } else if (typeof define === 'function' && define.amd) {
    define(function () {
      return Tabletop4;
    });
  } else {
    window.Tabletop4 = Tabletop4;
  }
