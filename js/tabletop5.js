(function() {
  ...
  /*
    Initialize with Tabletop5.init( { key: '0AjAPaAU9MeLFdHUxTlJiVVRYNGRJQnRmSnQwTlpoUXc' } )
      OR!
    Initialize with Tabletop5.init( { key: 'https://docs.google.com/spreadsheet/pub?hl=en_US&hl=en_US&key=0AjAPaAU9MeLFdHUxTlJiVVRYNGRJQnRmSnQwTlpoUXc&output=html&widget=true' } )
      OR!
    Initialize with Tabletop5.init('0AjAPaAU9MeLFdHUxTlJiVVRYNGRJQnRmSnQwTlpoUXc')
  */

  var Tabletop5 = function(options) {
    // Make sure Tabletop5 is being used as a constructor no matter what.
    if(!this || !(this instanceof Tabletop5)) {
      return new Tabletop5(options);
    }
    ...
    if (this.singleton) {
      if (typeof(Tabletop5.singleton) !== 'undefined') {
        this.log('WARNING! Tabletop5 singleton already defined');
      }
      Tabletop5.singleton = this;
    }
    ...
    if (!this.key) {
      this.log('You need to pass Tabletop5 a key!');
      return;
    }
  ...
  Tabletop5.init5 = function(options) {
    return new Tabletop5(options);
  };

  Tabletop5.sheets = function() {
    this.log('Times have changed! You\'ll want to use var Tabletop5 = Tabletop5.init(...); Tabletop5.sheets(...); instead of Tabletop5.sheets(...)');
  };
    ...
    injectScript: function(path, callback) {
      ...
      if (this.singleton) {
        if (callback === this.loadSheets) {
          callbackName = 'Tabletop5.singleton.loadSheets';
        } else if (callback === this.loadSheet) {
          callbackName = 'Tabletop5.singleton.loadSheet';
        }
      } else {
        var self = this;
        callbackName = 'tt' + (+new Date()) + (Math.floor(Math.random()*100000));
        Tabletop5.callbacks[ callbackName ] = function () {
          var args = Array.prototype.slice.call( arguments, 0 );
          callback.apply(self, args);
          script.parentNode.removeChild(script);
          delete Tabletop5.callbacks[callbackName];
        };
        callbackName = 'Tabletop5.callbacks.' + callbackName;
      }
  ...
  Tabletop5.Model = function(options) {
    var i, j, ilen, jlen;
    this.columnNames = [];
  ...
  if(typeof module !== 'undefined' && module.exports) {
    module.exports = Tabletop5;
  } else if (typeof define === 'function' && define.amd) {
    define(function () {
      return Tabletop5;
    });
  } else {
    window.Tabletop5 = Tabletop5;
  }
