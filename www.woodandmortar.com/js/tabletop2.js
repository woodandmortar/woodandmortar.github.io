(function() {
  ...
  /*
    Initialize with Tabletop2.init( { key: '0AjAPaAU9MeLFdHUxTlJiVVRYNGRJQnRmSnQwTlpoUXc' } )
      OR!
    Initialize with Tabletop2.init( { key: 'https://docs.google.com/spreadsheet/pub?hl=en_US&hl=en_US&key=0AjAPaAU9MeLFdHUxTlJiVVRYNGRJQnRmSnQwTlpoUXc&output=html&widget=true' } )
      OR!
    Initialize with Tabletop2.init('0AjAPaAU9MeLFdHUxTlJiVVRYNGRJQnRmSnQwTlpoUXc')
  */

  var Tabletop2 = function(options) {
    // Make sure Tabletop2 is being used as a constructor no matter what.
    if(!this || !(this instanceof Tabletop2)) {
      return new Tabletop2(options);
    }
    ...
    if (this.singleton) {
      if (typeof(Tabletop2.singleton) !== 'undefined') {
        this.log('WARNING! Tabletop2 singleton already defined');
      }
      Tabletop2.singleton = this;
    }
    ...
    if (!this.key) {
      this.log('You need to pass Tabletop2 a key!');
      return;
    }
  ...
  Tabletop2.init2 = function(options) {
    return new Tabletop2(options);
  };

  Tabletop2.sheets = function() {
    this.log('Times have changed! You\'ll want to use var Tabletop2 = Tabletop2.init(...); Tabletop2.sheets(...); instead of Tabletop2.sheets(...)');
  };
    ...
    injectScript: function(path, callback) {
      ...
      if (this.singleton) {
        if (callback === this.loadSheets) {
          callbackName = 'Tabletop2.singleton.loadSheets';
        } else if (callback === this.loadSheet) {
          callbackName = 'Tabletop2.singleton.loadSheet';
        }
      } else {
        var self = this;
        callbackName = 'tt' + (+new Date()) + (Math.floor(Math.random()*100000));
        Tabletop2.callbacks[ callbackName ] = function () {
          var args = Array.prototype.slice.call( arguments, 0 );
          callback.apply(self, args);
          script.parentNode.removeChild(script);
          delete Tabletop2.callbacks[callbackName];
        };
        callbackName = 'Tabletop2.callbacks.' + callbackName;
      }
  ...
  Tabletop2.Model = function(options) {
    var i, j, ilen, jlen;
    this.columnNames = [];
  ...
  if(typeof module !== 'undefined' && module.exports) {
    module.exports = Tabletop2;
  } else if (typeof define === 'function' && define.amd) {
    define(function () {
      return Tabletop2;
    });
  } else {
    window.Tabletop2 = Tabletop2;
  }
